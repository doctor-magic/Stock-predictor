...
**RESOLVED (Jun 18) — tracker.db 10-day resolver was NOT a bug:** The Jun 3 batch was simply pending one more run; the Jun 17 20:05 cron resolved it correctly (exit = Jun 17 close = full 10 trading days). The resolver counts actual trading days, not calendar days — working as intended. **Cross-validated** against an independent manual yfinance computation: same hit/miss direction on all 7 signals (magnitude differs only by the +1 exit day). tracker.db resolution is confirmed HEALTHY. (The 5-day `setup_resolve` burst/silent-skip above is a SEPARATE, still-open issue — do not conflate.)
