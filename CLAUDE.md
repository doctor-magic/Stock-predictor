...
- **Stage B sample-start amendment (LOCKED Jul 31 2026, blind):** `lev_spec_amendment_1.json` floors Stage B eligibility at **2026-07-27** — the Jul-26 warm-cron fix is a sample composition break (BC accrual ~1/wk in W28–W29 → 14 in W30), so pre/post-fix rows may not share one N=50 pool. Clock restarted at N=0; boundary (semis 0.37) and min_n unchanged. **Amendments are additive write-once files — never edit `lev_spec_frozen.json`.** Write one with `lev_sitting.py --amend-sample-start DATE --amend-reason "..."` (reads no DB column, so it cannot be outcome-informed).
